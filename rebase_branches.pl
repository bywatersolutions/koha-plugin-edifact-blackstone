...
say "\nNot koha-plugin-edifact-blackstone, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-blackstone";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-blackstone';
    next if $repo eq 'koha-plugin-edifact-blackstone-docs';
